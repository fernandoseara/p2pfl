...
NOTE: Many tests are skipped because VirtualLearnerActor inherits from LearnerDecorator
which extends Learner(ABC). Ray cannot serialize classes with ABC metaclass.
This is an architectural issue that requires removing ABC from the inheritance chain
or using a different actor pattern.
"""
...
# The following tests are skipped due to Ray serialization issues with ABC-based actors.
# The VirtualLearnerActor class inherits from LearnerDecorator(Learner(ABC, NodeComponent))
# and Ray's cloudpickle cannot serialize the ABC metaclass data.
#
# Error: TypeError: cannot pickle '_abc._abc_data' object
#
# To fix this, the production code would need to:
# 1. Remove ABC from the Learner class inheritance
# 2. Use a different pattern for enforcing abstract methods
# 3. Create a separate non-ABC class for Ray actors


@pytest.mark.skip(reason="VirtualLearnerActor has Ray serialization issues with ABC-based actors")
