...
class TrainingFinishedStage(Stage):
    """Round Finished Stage."""

    @staticmethod
    async def execute(state: NodeState, communication_protocol: CommunicationProtocol, learner: Learner) -> None:
        """Execute the stage."""
        # At end, all nodes compute metrics
        await TrainingFinishedStage.__evaluate(state, learner, communication_protocol)
        # Finish
        state.clear()
    ...
    @staticmethod
    async def __evaluate(state: NodeState, learner: Learner, communication_protocol: CommunicationProtocol) -> None:
